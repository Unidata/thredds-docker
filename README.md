...
```shell
ENV THREDDS_WAR_URL https://downloads.unidata.ucar.edu/tds/5.7/thredds-5.7.war
```
...
-   Create a new git branch corresponding to this version of the TDS (e.g., `5.7`).
...
-   Build a docker image corresponding to the new version of the TDS (e.g., on the Docker build machine on Jetstream). E.g., `docker build -t unidata/thredds-docker:5.7`.
-   Test to ensure the image works.
-   Push it out DockerHub e.g., `docker push unidata/thredds-docker:5.7`.
